if (vis && step > localNear && step < localFar)
{
    ...
    if(ambientOcclusion && x.a != 0.) {
        ...
        for(int ray = 0; ray < numAORays; ray++) {
            ...
            aoRays[ray] = (1. - sampleTransferFunction(vec4(wpos.xyz + dirs[ray] * 2 * minVoxelSize, 1)));

            for(int aoStep = 1; aoStep < numAOSteps; aoStep++) {
                aoRays[ray] = (aoRays[ray] + ((aoRays[ray] / aoStep) * (1. - sampleTransferFunction(vec4(wpos.xyz + dirs[ray] * (aoStep + 2) * minVoxelSize, 1)))));
            }
