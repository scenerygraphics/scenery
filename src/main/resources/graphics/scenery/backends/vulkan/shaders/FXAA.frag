...
void main()
{
    ...
    vec3 sampleCenter = texture(hdrBuffer, textureCoord).rgb;
    ...
    vec3 sampleNW = textureOffset(hdrBuffer, textureCoord, ivec2(-1, 1)).rgb;
    vec3 sampleNE = textureOffset(hdrBuffer, textureCoord, ivec2(1, 1)).rgb;
    vec3 sampleSW = textureOffset(hdrBuffer, textureCoord, ivec2(-1, -1)).rgb;
    vec3 sampleSE = textureOffset(hdrBuffer, textureCoord, ivec2(1, -1)).rgb;
    ...
    vec3 sampleNegative = texture(hdrBuffer, textureCoord + sampleDirection * (1.0/3.0 - 0.5)).rgb;
    vec3 samplePositive = texture(hdrBuffer, textureCoord + sampleDirection * (2.0/3.0 - 0.5)).rgb;

    vec3 twoTab = (samplePositive + sampleNegative) * 0.5;

    vec3 sampleNegativeOuter = texture(hdrBuffer, textureCoord + sampleDirection * (0.0/3.0 - 0.5)).rgb;
    vec3 samplePositiveOuter = texture(hdrBuffer, textureCoord + sampleDirection * (3.0/3.0 - 0.5)).rgb;
