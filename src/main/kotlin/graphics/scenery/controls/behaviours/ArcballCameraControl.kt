...
open class ArcballCameraControl(private val name: String, camera: () -> Camera?, private val w: Int, private val h: Int, var target: () -> Vector3f) : DragBehaviour, ScrollBehaviour,
    ...
    override fun drag(x: Int, y: Int) {
        cam?.let { node ->
            ...
            arcballRotate(xoffset, yoffset, node)
    ...
    /**
     *  This function rotates the [node] by a fixed [yaw] and [pitch] about the [target]
     *
     *  @param[yaw] yaw in degrees
     *  @param[pitch] pitch in degrees
     *  @param[node] the camera to move
     */
    private fun arcballRotate(yaw: Float, pitch: Float, node: Camera) {
