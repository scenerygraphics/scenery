...
/**
 * Lens cropping example on BDV volumes
 *
 * @author Ulrik Günther <hello@ulrik.is>
 * @author Samuel Pantze
 */
class BDVLensingExample: SceneryBase("BDV Lensing example", 1280, 720) {
    ...
    override fun init() {
        ...
        v.transferFunction = TransferFunction.ramp(0f, 0.15f)
        v.multiResolutionLevelLimits = 0 to 1

        // Switch between LensingSmooth and LensingHard
        v.slicingMode = Volume.SlicingMode.LensingSmooth
        v.lensingRadius = 0.2f

        var i = 0
        thread {
            while (true) {
                v.lensingPosition.x = sin(i / 100f) * 0.8f
                Thread.sleep(20)
                i += 1
            }
        }

        v.lensingPosition = Vector3f(0f)
