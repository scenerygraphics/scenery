...
/**
 * Tests for the [JOGLMouseAndKeyHandler].
 *
...
class JOGLMouseAndKeyHandlerTests {
    ...
    private fun prepareInputHandler(preparedWindow: SceneryWindow? = null): Triple<InputHandler, Scene, JOGLMouseAndKeyHandler> {
        ...
        val inputHandler = InputHandler(scene, renderer, h, forceHandler = JOGLMouseAndKeyHandler::class.java)

        return Triple(inputHandler, scene, inputHandler.handler as JOGLMouseAndKeyHandler)
    }
    ...
    fun testInitialisation() {
        ...
        val (inputHandler, scene, joglHandler) = prepareInputHandler()

        assertNotNull(inputHandler)
        assertNotNull(scene)
        assertNotNull(joglHandler)
    }
    ...
    fun testKeyPressedHandler() {
        ...
        val (inputHandler, _, joglHandler) = prepareInputHandler()
        ...
        repeat(100) {
            ...
            joglHandler.keyPressed(pressEvent)
            joglHandler.keyReleased(releaseEvent)
...
//                "alt",
            ...
            modifierEvents.forEach {
                joglHandler.keyPressed(it.first)
            }
            ...
            joglHandler.mouseClicked(clickEvent)

            modifierEvents.forEach {
                joglHandler.keyReleased(it.second)
            }
...
//                "alt",
            ...
            joglHandler.mouseWheelMoved(scrollEvent)
    ...
    fun testMouseDraggedHandlers() {
        ...
        repeat(100) { i ->
            val button = Random.nextInt(1, 3).toShort()
...
//                "alt",
            ...
            joglHandler.mousePressed(clickEvent)
            moveEvents.forEach { move ->
                joglHandler.mouseMoved(move)
                joglHandler.mouseDragged(move)
            }
            joglHandler.mouseReleased(releaseEvent)
    ...
    fun testUnsupportedWindowType() {
        ...
        assertFailsWith(UnsupportedOperationException::class, "Expeceted JOGLMouseAndKeyHandler not to support BrokenWindow") {
